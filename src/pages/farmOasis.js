...
function renderRule2(){
  return <div>
    ...
    <div>接下來開始說明的是觸發動物生成以後，動物生成的機制

    當觸發生怪以後，他一定會從時間最短的老鼠，開始判斷兩個問題
    問題一 要不要生老鼠 ?  
    不要 => 換判斷下一種動物(蜘蛛)要不要生成 。
    
    若問題一成立(要生) 則才會產生問題二，要生幾隻? 
    生完繼續判斷下一種動物(蜘蛛)要不要生成。
    
    以下舉例
    時間是下午13:00整，當綠洲觸發生怪後
    系統開始判斷
    順序一:  要不要生老鼠? 假設系統判定結果是 要
    順序二:  要生幾隻老鼠? 假設系統判定結果是 生5隻
    順序三:.接下來的時間
    13:05會產出第一隻老鼠，
    13:10會產出第二隻老鼠，
    13:15會產出第三隻老鼠，
    ...
    順序四:13:20生完老鼠的瞬間，系統馬上繼續判斷要不要生蜘蛛? 假設系統判定結果是 不要
    順序五:.馬上繼續判斷，要不要生毒蛇?  假設系統判定結果是 不要
    順序六:.馬上繼續判斷，要不要生蝙蝠?  假設系統判定結果是 不要
    順序七:.馬上繼續判斷，要不要生野豬?  假設系統判定結果是 要
    順序八: 系統接著判斷 要生幾隻野豬? 假設系統判定結果是 3隻
    順序九: 接下來的時間 
    13:29 會產出第一隻野豬 (如果綠洲怪都沒被清 這時候會有4隻老鼠+1隻野豬了)
    13:38 會產出第二隻野豬
    13:47 會產出第三隻野豬
    順序十:馬上繼續判斷，要不要生野狼?  假設系統判定結果是 不要
    順序十一:馬上繼續判斷，要不要生灰熊?  假設系統判定結果是 不要
    順序十二:馬上繼續判斷，要不要生鱷魚?  假設系統判定結果是 不要
    順序十三:馬上繼續判斷，要不要生老虎?  假設系統判定結果是 不要
    順序十四:馬上繼續判斷，要不要生大象?  假設系統判定結果是 不要
    
    此時，這一次的生怪循環 就到此告一個段落了。
    ...
    實際上這些只是"常見"動物，任何一種動物 在任何一種綠洲裡都是有機會生成的。
    ...
    3.一次生怪循環一定是從左到右判斷，生不生成&生成幾隻，
    意思是一定是從老鼠生不生開始判斷，最後結尾是大象生不生，絕不會回頭判斷。
    
    所以如果正在綠洲正在出野豬，出一出開始出熊了，則可以知道 野狼的生成被系統跳過了。
    當熊出完的時候，只有可能繼續生成鱷魚/老虎/大象/其中一種，
    倘若你發現這一個綠洲居然又開始生野豬了，那表示它已經開始下一次生怪循環了，等等生完野豬很有可能會繼續生野狼跟灰熊了恭喜。
    
    4.因為每一次生怪循環的結尾都一定會判斷大象生不生，所以簡單來說 當你發現當下一直有在刷到動物，資源一直跳 很爽，一堆綠洲都在瘋狂出怪的時候--------
